...
/**
 * These tests are, by their very nature, very hardware specific. Skynet circa 2029 probably will be able to complete
 * the exponential regexp in under 25 seconds, for example.
 */
...
public class BlockingRegexpDosExampleApplicationTests {
    ...
    /**
     * The classic ReDos example is simpler than this, but modern Java versions actually do a good job at getting though
     * power-2 complexity in regexps these days, so to demonstrate the problem power-3 or greater is used.
     * <p>
     * Since power-3 or worse regexps should be quite rare, this vulnerability is therefore most prevalent in services
     * that allow user-definable regexps for, say, custom field validation or the like.
     */
    ...
    public void smallRegexpDoesNotBlock() {
        testRegexp(
                SMALL_LINEAR_REGEXP,
                new MaxTimeLimit(Duration.ofSeconds(1))
        );
    }
    ...
    public void largeExponentialRegexpDoesBlock() {
        testRegexp(
                EVIL_REGEXP,
                new MinTimeLimit(Duration.ofSeconds(20))
        );
    }
}
